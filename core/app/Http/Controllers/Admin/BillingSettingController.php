...
class BillingSettingController extends Controller{
    ...
    public function update(Request $request){
        ...
        $billingSetting->create_default_invoice_days = $request->create_default_invoice_days;
        $billingSetting->invoice_send_reminder_days = $request->invoice_send_reminder_days;
        ...
        $billingSetting->invoice_late_fee_type = $request->invoice_late_fee_type;

        $billingSetting->invoice_send_reminder = $request->invoice_send_reminder ? 1 : 0;

        $billingSetting->save();

        $notify[] = ['success', 'Billing setting has been updated'];
        return back()->withNotify($notify);
    }

    public function updateBillingInvoice(Request $request){
        ...
        array_walk($data, function(&$field, $value) use ($request){
            $field = @$request->input(@$value) ?? "0"; 
        });

        $billingSetting->create_invoice = $data;

        $billingSetting->create_domain_invoice_days = $request->create_domain_invoice_days;
        $billingSetting->save();

        $notify[] = ['success', 'Invoice billing setting has been updated'];
        return back()->withNotify($notify);
    }
